...
st.set_page_config(page_title="Screeners & Watchlist Stable", page_icon="📈", layout="wide")
...
def sql_df(sql: str, params: dict | None = None, readonly: bool = True) -> pd.DataFrame:
    payload = {"sql": sql, "query": sql, "params": params or {}, "readonly": readonly}
...
def get_screener_source() -> pd.DataFrame:
    return sql_df(
        """
        SELECT TOP (1) *
        FROM (
            SELECT 1 AS SortOrder, 'dbo.vwCandlesScreener_Latest_Compat' AS ObjectName,
                CASE
            ...
            UNION ALL
            SELECT 2 AS SortOrder, 'dbo.vwCandlesScreener_Latest' AS ObjectName,
                CASE
            ...
            UNION ALL
            SELECT 3 AS SortOrder, 'dbo.Candles_Calculations' AS ObjectName,
                CASE
...
@st.cache_data(ttl=60)
def get_screener_run_dates() -> pd.DataFrame:
    return sql_df(
        """
        ...
        IF @ObjectName IS NULL OR @DateColumn IS NULL
        BEGIN
            SELECT CAST(NULL AS date) AS RunDate WHERE 1 = 0;
            RETURN;
        END

        SET @Sql = N'SELECT TOP (30) CAST(' + QUOTENAME(@DateColumn) + N' AS date) AS RunDate'
            + N' FROM ' + @ObjectName
            + N' GROUP BY CAST(' + QUOTENAME(@DateColumn) + N' AS date)'
            + N' ORDER BY RunDate DESC;';
...
st.title("Screeners & Watchlist Stable")
st.caption("Defaults to the latest stored screener date and keeps TradingView links front and center.")

source = get_screener_source()
run_dates = get_screener_run_dates()
run_date_options = pd.to_datetime(run_dates["RunDate"]).dt.date.tolist() if not run_dates.empty else []
latest_screener_date = run_date_options[0] if run_date_options else date.today()

with st.sidebar:
    selected_run_date = st.selectbox("Run date", run_date_options, index=0) if run_date_options else st.date_input("Run date", value=latest_screener_date)
...
if not source.empty:
    ...
    src_cols[0].metric("Source object", src.get("ObjectName", "Unknown"))
    src_cols[1].metric("Date column", src.get("DateColumn", "Unknown"))
    src_cols[2].metric("Latest stored date", str(latest_screener_date))

results = get_candle_results(str(selected_run_date), top_n)
queue = get_execution_watchlist(str(selected_run_date))

if results.empty:
    st.warning(f"No screener passes are stored for {selected_run_date}. Try the latest stored date: {latest_screener_date}.")

setup_options = sorted(str(v) for v in results["SetupType"].dropna().unique()) if not results.empty and "SetupType" in results.columns else []
direction_options = sorted(str(v) for v in results["Direction"].dropna().unique()) if not results.empty and "Direction" in results.columns else []
...
metric_cols[0].metric("Screener rows", len(filtered_display))
metric_cols[1].metric("Queue rows", len(queue))
metric_cols[2].metric("Latest screener date", str(latest_screener_date))
metric_cols[3].metric("Unique tickers", filtered["Ticker"].nunique() if not filtered.empty and "Ticker" in filtered.columns else 0)

board_tab, watchlist_tab = st.tabs(["Screener Board", "Watchlist"])

with board_tab:
    if filtered_display.empty:
        st.info("No screener rows to display for the selected filters and date.")
    else:
        st.data_editor(
            filtered_display,
